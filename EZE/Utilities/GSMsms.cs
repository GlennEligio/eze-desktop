    ...
    class GSMsms
    {
        ...
        public bool Connect()
        {
            if (gsmPort == null || !isConnected || !gsmPort.IsOpen)
            {
                ...
                {
                    Console.WriteLine(e);
                    MessageBox.Show("Failed to connect to GSM Module", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    isConnected = false;
                }
        ...
        private void readThreadStart()
        {
            try
            {
                List<MessagesTable> messages = new List<MessagesTable>();
                if (isConnected)
                {
                    ...
                    if (response.Contains("\r\nOK\r\n"))
                    {
                        ...
                        while (m.Success)
                        {
                            ...
                            MessagesTable newMessage = new MessagesTable() { Code = f, DateReceived = dt, Sender = c, IsSeen = false };
